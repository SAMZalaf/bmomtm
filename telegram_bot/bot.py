...
© Mohamad Zalaf 2025"""
        ...
        elif query.data == "numbers_service_nonvoip":
            # اختيار خدمة NonVoip
            logger.info(f"User {user_id} selected NonVoip service")
            try:
                conn = sqlite3.connect(DATABASE_FILE)
                await nonvoip_main_menu(update, context, conn)
                conn.close()
            except Exception as e:
                logger.error(f"Error opening NonVoip: {e}")
                language = get_user_language(user_id)
                await query.edit_message_text(
                    "❌ حدث خطأ في فتح خدمة NonVoip" if language == 'ar' else "❌ Error opening NonVoip service"
                )
            ...
            try:
                ...
                welcome_message = "🏠 القائمة الرئيسية\n\nاختر الخدمة المطلوبة:" if language == 'ar' else "🏠 Main Menu\n\nChoose the required service:"
                
                await query.edit_message_text(welcome_message)
                await query.message.reply_text(
                    "تفضل، اختر من القائمة:" if language == 'ar' else "Please choose from the menu:",
...
async def handle_buy_numbers(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """معالج زر شراء الأرقام للمستخدمين - يوفر اختيار بين NonVoip و SMSPool"""
    user_id = update.effective_user.id
    language = get_user_language(user_id)
    
    # التحقق من توفر الخدمات
    nonvoip_available = NONVOIP_AVAILABLE and db.get_service_status('nonvoip', 'basic')
    smspool_available = SMSPOOL_AVAILABLE and smspool_db.is_enabled() if SMSPOOL_AVAILABLE else False
    
    # إذا لم تكن أي خدمة متاحة
    if not nonvoip_available and not smspool_available:
        message = "❌ خدمة الأرقام غير متاحة حالياً.\nيرجى التواصل مع الآدمن." if language == 'ar' else "❌ Numbers service is not available.\nPlease contact admin."
        await update.message.reply_text(message)
        return
    
    # إذا كانت خدمة واحدة فقط متاحة، افتحها مباشرة
    if nonvoip_available and not smspool_available:
        try:
            conn = sqlite3.connect(DATABASE_FILE)
            await nonvoip_main_menu(update, context, conn)
            conn.close()
        except Exception as e:
            logger.error(f"خطأ في شراء الأرقام NonVoip: {e}")
            await update.message.reply_text(f"❌ حدث خطأ: {str(e)}")
        return
    
    if smspool_available and not nonvoip_available:
        try:
            await smspool_main_menu(update, context)
        except Exception as e:
            logger.error(f"خطأ في شراء الأرقام SMSPool: {e}")
            await update.message.reply_text(f"❌ حدث خطأ: {str(e)}")
        return
    
    # كلا الخدمتين متاحتان - عرض قائمة اختيار
    if language == 'ar':
        text = """📱 <b>خدمات الأرقام المتاحة</b>

اختر الخدمة المطلوبة:

🔸 <b>NonVoip</b>: أرقام أمريكية بشكل أساسي
🔸 <b>SMSPool</b>: أرقام من جميع الدول"""
    else:
        text = """📱 <b>Available Number Services</b>

Choose the required service:

🔸 <b>NonVoip</b>: Mainly US numbers
🔸 <b>SMSPool</b>: Numbers from all countries"""
    
    keyboard = []
    if nonvoip_available:
        keyboard.append([InlineKeyboardButton(
            "🇺🇸 NonVoip" if language == 'ar' else "🇺🇸 NonVoip",
            callback_data="numbers_service_nonvoip"
        )])
    if smspool_available:
        keyboard.append([InlineKeyboardButton(
            "🌍 SMSPool" if language == 'ar' else "🌍 SMSPool",
