...
- Rentals: GET https://api.smspool.net/request/rental
- Purchase Rental: POST https://api.smspool.net/purchase/rental
- Rental Messages: POST https://api.smspool.net/sms/rental_code
- Extend Rental: POST https://api.smspool.net/purchase/extend
- Refund Rental: POST https://api.smspool.net/sms/refund_rental
- Archive Orders: POST https://api.smspool.net/request/archive
"""
...
class SMSPoolAPI:
    ...
    def archive_orders(self) -> Dict[str, Any]:
        """
        أرشفة جميع الطلبات المنتهية
        
        Returns:
            {"success": 1, "message": "Orders archived"}
        """
        result = self._api_request("request/archive")
        
        if result.get('success') == 1:
            return {"status": "success", "message": "Orders archived successfully"}
        
        return {"status": "error", "message": result.get('message', 'Archive failed')}
    
    # ════════════════════════════════════════════════════════════════════════
    # RENTALS API - الأرقام طويلة الأمد (شهرية)
    # ════════════════════════════════════════════════════════════════════════
    
    def get_rentals(self, rental_type: str = 'any') -> List[Dict]:
        """
        جلب الأرقام المتاحة للإيجار
        
        Args:
            rental_type: نوع الإيجار ('any', 'sms', 'call')
        
        Returns:
            قائمة الأرقام المتاحة للإيجار مع الأسعار
            [
                {
                    "ID": 123,
                    "number": "+1234567890",
        ...
        data = {}
        if rental_type != 'any':
            data['type'] = rental_type
        
        result = self._api_request("request/rental", method="GET", data=data)
        
        if isinstance(result, list):
            return result
        elif isinstance(result, dict) and 'rentals' in result:
            return result['rentals']
        
        return []
    
    def purchase_rental(self, rental_id: int, days: int, service_id: str) -> Dict[str, Any]:
        """
        استئجار رقم لفترة محددة
        
        Args:
            rental_id: معرف الرقم المتاح للإيجار
            days: عدد الأيام (1-30)
            service_id: معرف الخدمة المطلوبة
        ...
        result = self._api_request("purchase/rental", data=data)
        
        if result.get('success') == 1:
            return {
                "status": "success",
                "rental_code": result.get('rental_code'),
                "number": result.get('number'),
                "expires": result.get('expires'),
                "cost": result.get('cost'),
                "message": result.get('message', 'Rental purchased successfully')
            }
        
        return {
            "status": "error",
            "message": result.get('message', 'Rental purchase failed')
        }
    
    def get_rental_messages(self, rental_code: str) -> List[Dict]:
        """
        جلب الرسائل المستلمة على الرقم المستأجر
        
        Args:
            rental_code: كود الإيجار (مثل: ABC123)
        
        Returns:
            قائمة الرسائل (حد أقصى 25 رسالة يومياً)
            [
                {
                    "ID": 1,
                    "sms": "Your code is 123456",
        ...
        """
        result = self._api_request("sms/rental_code", data={'rental_code': rental_code})
    ...
    def get_rental_status(self, rental_code: str) -> Dict[str, Any]:
        """
        فحص حالة الإيجار
        
        Args:
            rental_code: كود الإيجار
        ...
        """
        result = self._api_request("request/rental_status", data={'rental_code': rental_code})
        ...
        if result.get('success') == 1:
            return {
                ...
                "number": result.get('number'),
                "rental_status": result.get('status', 'active'),
                "expires": result.get('expires'),
                "messages_today": result.get('messages_today', 0),
                "messages_total": result.get('messages_total', 0)
            }
        
        return {"status": "error", "message": result.get('message', 'Unknown error')}
    
    def extend_rental(self, rental_code: str, days: int) -> Dict[str, Any]:
        """
        تمديد فترة الإيجار
        
        Args:
            rental_code: كود الإيجار
            days: عدد الأيام الإضافية (1-30)
        
        Returns:
            {
                "success": 1,
                "new_expiry": "2024-03-01 00:00:00",
                "cost": "15.00"
            }
        """
        data = {
        ...
        result = self._api_request("purchase/extend", data=data)
        
        if result.get('success') == 1:
            return {
                "status": "success",
                "new_expiry": result.get('expires', result.get('new_expiry')),
                "cost": result.get('cost'),
                "message": f"Rental extended for {days} days"
            }
        
        return {
            "status": "error",
            "message": result.get('message', 'Extension failed')
        }
    
    def refund_rental(self, rental_code: str) -> Dict[str, Any]:
        """
        استرداد المبلغ وإلغاء الإيجار
        
        Args:
            rental_code: كود الإيجار
        
        Returns:
            {"success": 1, "refund_amount": "10.00"}
        """
        result = self._api_request("sms/refund_rental", data={'rental_code': rental_code})
...
class SMSPoolDB:
    ...
    def init_tables(self):
        ...
        # جدول الأرقام المستأجرة (Rentals)
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS smspool_rentals (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                user_id INTEGER NOT NULL,
        ...
        # جدول الرسائل (لحفظ آخر 3 رسائل لكل طلب/إيجار)
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS smspool_messages (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                order_id TEXT,
                rental_code TEXT,
                user_id INTEGER NOT NULL,
        ...
        # جدول العمليات (لتسجيل جميع الأحداث)
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS smspool_operations_log (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                user_id INTEGER NOT NULL,
        ...
        # جدول الإحصائيات
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS smspool_statistics (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        # جدول الإشعارات المرسلة (لمنع التكرار)
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS smspool_sent_notifications (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                order_id TEXT,
                rental_code TEXT,
                user_id INTEGER NOT NULL,
        ...
        # Indexes للأداء
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_smspool_orders_user ON smspool_orders(user_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_smspool_orders_status ON smspool_orders(status)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_smspool_rentals_user ON smspool_rentals(user_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_smspool_rentals_status ON smspool_rentals(status)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_smspool_messages_order ON smspool_messages(order_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_smspool_messages_rental ON smspool_messages(rental_code)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_smspool_operations_user ON smspool_operations_log(user_id)")
        ...
        conn.commit()
        conn.close()
        logger.info("SMSPool database tables initialized with full functionality")
...
# تطبيق التوسعات الإضافية (Rentals, Messages, Logging, Statistics)
try:
    from smspool_extensions import apply_all_extensions
    apply_all_extensions(SMSPoolDB)
    logger.info("✅ SMSPool extensions loaded successfully")
except ImportError as e:
    logger.warning(f"⚠️ SMSPool extensions not loaded: {e}")
